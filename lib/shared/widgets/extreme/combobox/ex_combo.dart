...
class ExComboState extends State<ExCombo> {
  ...
  initItems(starredItems) async {
    ...
    if (widget.initalValue != null) updateValueByValue(widget.initalValue);
    Input.comboController[widget.id] = this;
    if (this.mounted) {
      setState(() {});
    }
  }
  ...
  updateValueByValue(String value) {
    try {
      var sv = comboItems.firstWhere((item) => item["value"] == value);
